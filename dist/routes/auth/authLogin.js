...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        const user = await schemaUser_1.default.findOne({ USER_EMAIL });
        if (!user) {
        ...
        const isPasswordValid = await argon2_1.default.verify(user.USER_PASSWORD, USER_PASSWORD);
        if (!isPasswordValid) {
            return res.status(400).json({
                error: 'Auth failed',
                message: 'Auth failed due to an internal server error. Please try again later.',
        ...
        if (USER_ADMIN === true) {
            const token = jsonwebtoken_1.default.sign({ id: user._id }, secret, { expiresIn: '20h' });
            const tokenAdmin = jsonwebtoken_1.default.sign({ id: user._id }, secretAdmin, { expiresIn: '20h' });
